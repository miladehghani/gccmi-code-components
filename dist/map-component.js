...
export default function Mapbox(props) {
  ...
  const width =
    size !== null && size !== void 0 && size.width ? size.width : 400;
  const height =
    size !== null && size !== void 0 && size.height ? size.height : 300;
  const {
    style,
    locationType,
    city,
    ...
    map.current.on("load", (e) => {
      ...
      Object.entries(cityCoordinates).forEach(([key, val]) => {
        ...
        const popup = new mapboxgl.Popup({
          offset: 25,
        }).setHTML(`<h3>${key}</h3><button id="${id}">view data</button>`);
        new mapboxgl.Marker()
